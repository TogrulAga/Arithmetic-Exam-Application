...
class ExamTest(StageTest):
    ...
    def test_level_2_numbers(self):
        ...
        if numbers == set(range(start, finish+1)):
            ...
            return CheckResult.wrong('If the user chooses level 2 training, the app should use all numbers from 11 to 29\n'
    ...
    def test_no_saving(self):
        ...
        if os.path.exists("results.txt"):
            os.remove('results.txt')
            return CheckResult.wrong('The app created the file for results, even though user hasn\'t answered "yes" or "y" (in any register).')

        if not pr.is_waiting_input():
            return CheckResult.correct()
        else:
            return CheckResult.wrong('The app is waiting for the input after the user answered that the results shouldn\'t be saved to the file.\n'
